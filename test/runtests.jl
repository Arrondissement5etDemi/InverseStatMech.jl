using InvStatMech, SpecialFunctions
using Test

@testset "reverse_monte_carlo" begin
    kF = 2*√π
    targ_g2(r) = 1 - 4*besselj(1, kF*r)^2/(kF*r)^2
    @test InvStatMech.reverse_mc(2, 500, 1, targ_g2; test = true)
end
...
@testset "torquato-wang algorithm" begin
    ...
    @test InvStatMech.optim_parametrized_pot(params_vec, pot, 2, 1.0, targ_g2, targ_s; Ψ_tol = 0.01, test = true)
end
