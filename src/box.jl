...
mutable struct Box
    ...
    function Box(_particles, _l, _bin_size, _range)
        ...
        this.compute_s = function(k_range = 20, k_bin_size = 0.05)
            return struc_fac(this.particles, this.l, k_range, k_bin_size) #see helper_funcs.jl
        end
        ...
        this.visualize = function()
            ...
            if this.dim() == 2
                return Plots.scatter(x, y, markersize = 0.5, aspect_ratio = :equal)
            elseif this.dim() == 3
                z = vec(this.particles[3, :])
                return Plots.scatter(x, y, z, markersize = 0.5, aspect_ratio = :equal)
            end
        end
        return this
    end
...
function g2_boxes(boxarr)
    ...
    return reweighed_f(weights_uniform, g2_arr)
end
...
function s_boxes(boxarr, k_range = 20, k_bin_size = 0.05)
    ...
    return reweighed_f(weights_uniform, s_arr)
end

```obtain an ensemble of simulated boxes```
function simu_boxes(pair_pot, pot_params, dim, n, ρ, bin_size, range, large_r_grid, thread_arr = missing; 
        n_threads = 20, configs_per_thread = 5, displace = 0.2, show_pb)
    box_arr = Vector{Box}(undef, n_threads*configs_per_thread)
    if ismissing(thread_arr)
        thread_arr = Vector{Box}(undef, n_threads)
        b = random_box(dim, n, ρ, bin_size, range)
        pair_pot_faster = equilibrate!(b, pair_pot, pot_params, false, large_r_grid; sweeps = 600, displace = displace, show_pb = show_pb) #initial equilibration
        for i in eachindex(thread_arr)
            thread_arr[i] = deepcopy(b)
        end
    else
        pair_pot_faster = equilibrate!(thread_arr[1], pair_pot, pot_params, false, large_r_grid; sweeps = 1, displace = displace, show_pb = show_pb)
    end
    if show_pb
        p = Progress(n_threads*configs_per_thread)
        @sync for i in 1:n_threads
            Threads.@spawn for j = 1:configs_per_thread
                equilibrate!(thread_arr[i], pair_pot, pot_params, false, large_r_grid, pair_pot_faster; displace = displace, sweeps = 50)
                box_arr[(i - 1)*configs_per_thread + j] = deepcopy(thread_arr[i])
                next!(p)
                flush(stdout)
            end
        end
